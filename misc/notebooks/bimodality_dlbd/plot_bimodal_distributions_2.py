...
def plot_bimodal_distributions(file_path, x=10):
    """
    Plots an x×x grid of randomly selected bimodal distributions from a CSV file.
    Each histogram bin has equal width.

    Args:
        file_path (str): Full path to the CSV file
        x (int): Number of rows and columns in the grid (default: 10)
    """
    ...
    # Calculate the target sample size based on x
    target_sample_size = x * x
    ...
    if len(df) < target_sample_size:
        ...
        print(f"Found {len(df)} distributions, randomly sampling {target_sample_size}.")

    # Randomly sample the distributions
    sample_indices = random.sample(range(len(df)), sample_size)
    sample_df = df.iloc[sample_indices].reset_index(drop=True)

    # Create an x×x grid (or adjust if fewer samples)
    rows = cols = x
    if sample_size < target_sample_size:
        rows = cols = int(np.ceil(np.sqrt(sample_size)))

    # Set up the figure
    fig, axes = plt.subplots(rows, cols, figsize=(2 * x, 2 * x))  # Scale figure size with x
    ...
    output_filename = os.path.join(file_dir, f"{file_pattern}_equal_width_hist_{x}x{x}.png")
...
file_path = "/home/jan/Programming/remote/first_CNN_on_Radolan/notebooks/bimodality_dlbd/dlbd_results/bimodal_distributions_20250520_093909_sigma_10.csv"

# Run the function with default 10x10 grid
plot_bimodal_distributions(file_path, x=5)

# Example: Run with a 5x5 grid instead
# plot_bimodal_distributions(file_path, x=5)