#!/bin/bash
#SBATCH --ntasks=1                # Number of tasks (see below)
#SBATCH --cpus-per-task=1         # Number of CPU cores per task
#SBATCH --nodes=1                 # Ensure that all cores are on one machine
#SBATCH --time=0-05:00            # Runtime in D-HH:MM
#SBATCH --gres=gpu:2    # optionally type and number of gpus
#SBATCH --mem=200G                # Memory pool for all cores (see also --mem-per-cpu)
#SBATCH --output=out.out  # File to which STDOUT will be written - make sure this is not on $HOME
#SBATCH --error=err.err   # File to which STDERR will be written - make sure this is not on $HOME
#SBATCH --mail-type=END           # Type of email notification- BEGIN,END,FAIL,ALL
#SBATCH --mail-user=jan-gerhard.prosi@uni-tuebingen.de   # Email to which notifications will be sent

# insert your commands here
#source venv/bin/activate
python3 train.py
